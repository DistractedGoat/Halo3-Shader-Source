...
*/
	...
	// Scale blur offsets to maintain consistent screen-space bloom radius at any resolution.
	// Reference: Xbox 360 bloom buffer (1152x640 / 2 = 576x320).
	// At higher resolutions the bloom buffer has more texels, so the fixed 11-texel kernel
	// covers a smaller screen fraction.  blur_scale compensates by widening the offsets.
	float2 blur_scale = max(1.0, float2(1.0/576.0, 1.0/320.0) / ps_postprocess_pixel_size.xy);
	float4 scaled_pixel_size = ps_postprocess_pixel_size * blur_scale.xyxy;

	float4 color=	(1.0   + 9.0)	* sample2D(target_sampler, sample + offset[0] * scaled_pixel_size) +
					(36.0  + 84.0)	* sample2D(target_sampler, sample + offset[1] * scaled_pixel_size) +
					(126.0 + 126.0)	* sample2D(target_sampler, sample + offset[2] * scaled_pixel_size) +
					(84.0  + 36.0)	* sample2D(target_sampler, sample + offset[3] * scaled_pixel_size) +
					(1.0   + 9.0)	* sample2D(target_sampler, sample + offset[4] * scaled_pixel_size);
